...
public class Appointment{
 ...
 public static Appointment pending(UUID patient,UUID patientIdentity,UUID doctor,UUID doctorIdentity,Instant start,Instant end,String reason,String key,BigDecimal consultationFeeSnapshot){
  var x=new Appointment();x.id=UUID.randomUUID();x.patientId=patient;x.patientIdentityId=patientIdentity;x.doctorId=doctor;x.doctorIdentityId=doctorIdentity;x.startAt=start;x.endAt=end;x.reason=reason;x.idempotencyKey=key;x.consultationFeeSnapshot=consultationFeeSnapshot;x.status=doctor==null?AppointmentStatus.PENDING:AppointmentStatus.ASSIGNED;return x;
 }
 public static Appointment held(UUID patient,UUID patientIdentity,UUID doctor,UUID doctorIdentity,Instant start,Instant end,BigDecimal consultationFeeSnapshot){
  var x=pending(patient,patientIdentity,doctor,doctorIdentity,start,end,null,null,consultationFeeSnapshot);x.status=AppointmentStatus.HELD;x.holdExpiresAt=Instant.now().plus(Duration.ofMinutes(5));return x;
 }
 public static Appointment proposed(UUID patient,UUID patientIdentity,UUID doctor,UUID doctorIdentity,Instant start,Instant end,String reason,BigDecimal consultationFeeSnapshot){
  var x=pending(patient,patientIdentity,doctor,doctorIdentity,start,end,reason,null,consultationFeeSnapshot);x.status=AppointmentStatus.PROPOSED;x.holdExpiresAt=Instant.now().plus(Duration.ofMinutes(10));return x;
 }
