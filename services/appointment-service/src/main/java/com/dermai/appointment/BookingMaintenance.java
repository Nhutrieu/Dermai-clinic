...
@Component class BookingMaintenance{
 ...
 public void expireHolds(){var expired=appointments.findByStatusAndHoldExpiresAtBefore(AppointmentStatus.HELD,Instant.now());if(expired.isEmpty())return;expired.forEach(x->{x.status=AppointmentStatus.CANCELLED;x.cancelReason="HOLD_EXPIRED";x.patientHidden=true;x.updatedAt=Instant.now();});appointments.saveAll(expired);slots.afterCommit();}
