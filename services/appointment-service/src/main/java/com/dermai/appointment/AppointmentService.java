...
public class AppointmentService{
 ...
 public Appointment checkIn(UUID id){
  var x=locked(id);var clinicZone=ZoneId.of("Asia/Ho_Chi_Minh");
  // Check-in records physical arrival independently from the doctor's start action.
  if(!LocalDate.ofInstant(x.startAt,clinicZone).equals(LocalDate.now(clinicZone)))throw new IllegalStateException("CHECK_IN_ONLY_ON_APPOINTMENT_DATE");
  x.transition(AppointmentStatus.CHECKED_IN);x.checkedInAt=Instant.now();repo.save(x);event(x,"AppointmentCheckedIn");notify(x,"CHECKED_IN","Đã tiếp nhận tại phòng khám","Lễ tân đã xác nhận bạn có mặt. Vui lòng chờ đến lượt khám.");slots.afterCommit();return x;
 }
 public Appointment assign(UUID id,UUID doctor,UUID doctorIdentity,Instant start,Instant end,BigDecimal consultationFeeSnapshot){var x=locked(id);if(x.status!=AppointmentStatus.PENDING)throw new IllegalStateException("INVALID_TRANSITION");if((start==null)!=(end==null))throw new IllegalArgumentException("START_AND_END_REQUIRED_TOGETHER");if(start!=null){if(!start.isBefore(end)||start.isBefore(Instant.now()))throw new IllegalArgumentException("INVALID_INTERVAL");x.startAt=start;x.endAt=end;}requireFeeWhenDoctorSelected(doctor,consultationFeeSnapshot);bookingPolicy.validateDoctorAssignment(x.id,x.patientIdentityId,doctor,x.startAt);x.doctorId=doctor;x.doctorIdentityId=doctorIdentity;x.consultationFeeSnapshot=consultationFeeSnapshot;x.transition(AppointmentStatus.ASSIGNED);flushConflict(x);event(x,"AppointmentAssigned");slots.afterCommit();return x;}
 public Appointment transition(UUID id,AppointmentStatus target){var x=locked(id);x.transition(target);repo.save(x);event(x,"Appointment"+target);if(target==AppointmentStatus.CONFIRMED)notify(x,"CONFIRMED","Lịch khám đã được xác nhận","Lễ tân đã xác nhận lịch khám của bạn.");if(target==AppointmentStatus.COMPLETED)notify(x,"COMPLETED","Buổi khám đã hoàn thành","Bạn có thể đánh giá trải nghiệm tại phòng khám.");slots.afterCommit();return x;}
 public Appointment completeStaleConsultation(UUID id){
  var x=locked(id);
  // Staff may correct a forgotten doctor action only after a one-hour grace period.
  if(x.status!=AppointmentStatus.IN_PROGRESS)throw new IllegalStateException("INVALID_TRANSITION");
  if(x.endAt.plus(Duration.ofHours(1)).isAfter(Instant.now()))throw new IllegalStateException("STALE_COMPLETION_TOO_EARLY");
