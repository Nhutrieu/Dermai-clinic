...
public class AppointmentService{
 ...
 public Appointment book(UUID patient,UUID patientIdentity,UUID doctor,UUID doctorIdentity,Instant start,Instant end,String reason,String key,boolean bypassActiveLimit){
 ...
 public Appointment followUp(UUID parent,Instant start,Instant end,String key,boolean bypassActiveLimit){var old=locked(parent);if(old.status!=AppointmentStatus.FOLLOW_UP_REQUIRED)throw new IllegalStateException("FOLLOW_UP_NOT_REQUIRED");if(old.followUpNotBefore!=null&&start.isBefore(old.followUpNotBefore))throw new IllegalArgumentException("FOLLOW_UP_TOO_EARLY");var next=book(old.patientId,old.patientIdentityId,old.doctorId,old.doctorIdentityId,start,end,old.followUpReason,key,bypassActiveLimit);next.parentId=old.id;old.status=AppointmentStatus.COMPLETED;event(next,"FollowUpBooked");return next;}
