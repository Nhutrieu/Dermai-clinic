...
function Login({ onLogin, onForgotPassword, onAccountLocked, notice = "" }: { onLogin: (tokens: Tokens) => void; onForgotPassword: () => void; onAccountLocked: () => void; notice?: string }) {
    ...
    return <div className="auth-page"><form className="auth-card" onSubmit={submit}>
        ...
        <label>Email<input type="email" value={email} onChange={event => setEmail(event.target.value)} required onInput={event => event.currentTarget.setCustomValidity("")} onInvalid={event => event.currentTarget.setCustomValidity(event.currentTarget.validity.valueMissing ? "Vui lòng nhập email." : "Email chưa đúng định dạng. Ví dụ: ten@domain.com.")} /></label>
        ...
        <button type="button" className="auth-switch" onClick={() => { setRegister(!register); setError(""); }}>{register ? "Đã có tài khoản? Đăng nhập" : "Chưa có tài khoản? Đăng ký bệnh nhân"}</button>
