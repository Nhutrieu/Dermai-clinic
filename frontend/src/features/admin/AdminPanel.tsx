...
function ClinicClosureManager({ token }: { token: string }) {
    const [items, setItems] = useState<ClinicClosure[]>([]); const [date, setDate] = useState(""); const [reason, setReason] = useState(""); const [message, setMessage] = useState(""); const [messageError, setMessageError] = useState(false);
    ...
    async function add(e: FormEvent) { e.preventDefault(); try { await request("/appointments/closures", token, { method: "POST", body: JSON.stringify({ date, reason }) }); setDate(""); setReason(""); setMessageError(false); setMessage("Đã thêm ngày phòng khám nghỉ."); await load() } catch (x) { setMessageError(true); setMessage((x as Error).message) } }
    async function remove(id: string) { try { await request(`/appointments/closures/${id}`, token, { method: "DELETE" }); setMessageError(false); await load() } catch (x) { setMessageError(true); setMessage((x as Error).message) } }
    return <section className="panel management clinic-closure-panel"><h2>Ngày phòng khám nghỉ</h2><p>Ngày nghỉ sẽ khóa lịch toàn bộ bác sĩ. Nếu đã có lịch đang hoạt động, hãy đổi hoặc hủy các lịch đó trước.</p><div className="clinic-closure-list">{items.length === 0 ? <p>Chưa khai báo ngày nghỉ chung.</p> : items.map(x => <article key={x.id}><div><b>{new Date(`${x.closureDate}T00:00:00`).toLocaleDateString("vi-VN")}</b><small>{x.reason}</small></div><button type="button" aria-label={`Xóa ngày nghỉ ${x.closureDate}`} onClick={() => remove(x.id)}><Trash2 />Xóa</button></article>)}</div><form onSubmit={add}><label>Ngày nghỉ<input type="date" required min={new Date().toISOString().slice(0, 10)} value={date} onChange={e => setDate(e.target.value)} /></label><label>Lý do<input required maxLength={300} value={reason} onChange={e => setReason(e.target.value)} placeholder="Ví dụ: Nghỉ lễ Quốc khánh" /></label><button className="primary">Thêm ngày nghỉ</button></form>{message && <p className={messageError ? "clinic-closure-message is-error" : "clinic-closure-message"} role={messageError ? "alert" : "status"}>{message}</p>}</section>
}
