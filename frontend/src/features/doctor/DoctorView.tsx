        ...
        <div className="consultation-backdrop">
            ...
            >
                ...
                {!recordId ? (
                    ...
                    <div className="prescription-stage">
                        <h3>Đơn thuốc — chỉ bác sĩ</h3>
                        {!prescriptionSigned && (
                            <form onSubmit={savePrescription}>
                                {items.map((item, index) => (
                                    <div className="drug-row" key={index}>
                                        <label>Tên thuốc<input required value={item.drugName} onChange={e => updateItem(index, "drugName", e.target.value)} /></label>
                                        <label>Liều dùng<input required value={item.dosage} onChange={e => updateItem(index, "dosage", e.target.value)} /></label>
                                        <label>Tần suất<input value={item.frequency} onChange={e => updateItem(index, "frequency", e.target.value)} /></label>
                                        <label>Thời gian dùng<input value={item.duration} onChange={e => updateItem(index, "duration", e.target.value)} /></label>
                                        <label>Hướng dẫn riêng<input value={item.instructions} onChange={e => updateItem(index, "instructions", e.target.value)} /></label>
                                        {items.length > 1 && <button type="button" onClick={() => setItems(items.filter((_, i) => i !== index))}>Xóa thuốc</button>}
                                    </div>
                                ))}
                                <label>Hướng dẫn chung<textarea value={rxInstructions} onChange={e => setRxInstructions(e.target.value)} /></label>
                        ...
                        <div className="finish-visit">
                            <p>{needFollowUp ? `Bác sĩ yêu cầu tái khám từ ngày ${new Date(`${followDate}T00:00:00`).toLocaleDateString("vi-VN")}; bệnh nhân tự chọn giờ.` : "Không yêu cầu tái khám"}</p>
                            <button className="primary" disabled={busy} onClick={finish}>{busy ? "Đang hoàn thành..." : prescriptionSigned ? "Hoàn thành ca khám" : "Hoàn thành không kê đơn"}</button>
                        </div>
                    </div>
                )}
